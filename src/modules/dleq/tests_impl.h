...
static void run_test_dleq_prove_verify(void) {
    ...
    }
    {
        ...
        CHECK(secp256k1_dleq_verify_internal(secp256k1_get_hash_context(CTX), &s, &e_one, &A_neg, &B, &B, msg) == 0);
    }
    {
        secp256k1_context *ctx = secp256k1_context_clone(CTX);
        ctx->hash_ctx.fn_sha256_compression = dleq_sha256_zeros;
        CHECK(secp256k1_dleq_prove_internal(ctx, &s, &e, &a, &B, &A, &C, aux_rand, msg) == 0);
